...
function dbReady(res) {
  if (!articlesCol) {
    res.status(503).json({ error: 'Database not connected. Set MONGODB_URI.' });
    return false;
  }
  return true;
}
...
connectDB().catch(e => console.log('  ✗ DB connection error:', e.message)).finally(() => {
  app.listen(PORT, () => {
    console.log('========================================');
    console.log('  glitchory is running!');
